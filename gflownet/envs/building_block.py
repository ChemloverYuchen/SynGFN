...
from data.template import smarts_list, name_list
from gflownet.utils.common import copy

global smiles_list, mask_dict, vocab_2, inverse_lookup_2, lookup_2, react_key

#Please note that when you need to adjust the block library scale, update the path here accordingly. The same applies to mask_dict.
...
class BuildingBlock:
    ...
    def statebatch2oracle(self, states: List[List]):
        product_smiles_list = [self.state2readable(s) for s in states]
        return product_smiles_list

    def state2policy_1(self, state: List = None):
        if state is None:
            state = self.state.copy()
        state_readable = self.state2readable(state)
    ...
    def state2readable(self, state: List):
        if state[0] == self.padding_idx:
            return None
        #if only one block
    ...
    def _pre_step(
        ...
        # If forward and env is done, step should not proceed.
            ...
            if self.done or self.state[2] != self.padding_idx:
                ...
                if self.get_mask_invalid_actions_forward_1()[action_1[0]] or self.get_mask_invalid_actions_forward_2(action_1=action_1)[action_2[0]]:
                    return False
                else:
                    return True
    ...
    def step(self, action_1: Tuple[int], action_2: Tuple[int],backward: bool = False):
        ...
        # if forward
            ...
            # if initial state and action_2 is not eos    
                ...
                self.n_actions += 1
                return self.state, action_1, action_2, True
            # if action_1 or action_2 is eos, then force eos
            ...
            #condition1:only one block,need to add second block to form intermidiate product
                ...
                state_next = self.state.copy()
                state_next[4] = self.get_uni_product_idx(self.state2readable(state_next),action_1[0])
                if state_next[4] == -1:
                    self.set_state(self.state, done=True)
                    valid = False 
                ...
                state_next = self.state.copy()
                state_next[4] = self.get_bi_product_idx(self.state2readable(state_next),action_1[0],action_2[0])
                if state_next[4] == -1:
                    self.set_state(self.state,done=True)
                    valid = False
                    return self.state, action_1, action_2, valid
            ...
            #condition2:two block,need to add third block to form final product
                ...
                state_next = self.state.copy()
                state_next[6] = self.get_uni_product_idx(self.state2readable(state_next),action_1[0])
                if state_next[6] == -1:
                    self.set_state(self.state,done=True)
                    valid = False
                    return self.state, action_1, action_2, valid
                ...
                state_next = self.state.copy()
                state_next[6] = self.get_bi_product_idx(self.state2readable(state_next),action_1[0],action_2[0])
                if state_next[6] == -1:
                    self.set_state(self.state,done=True)
                    valid = False
                    return self.state, action_1, action_2, valid
