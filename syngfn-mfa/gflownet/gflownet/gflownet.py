...
from .envs.building_block import BuildingBlock
from .policy.building_block import Policy
...
class GFlowNetAgent:
    def __init__(
        ...
        **kwargs,
        ...
        # Buffers
        self.replay_sampling = replay_sampling # "permutation"
        self.buffer = Buffer(
    ...
    def sample_batch(
        ...
        while len(envs) > 0:
            ...
            actions_1 = self.sample_actions_1(
                ...
                #no_random=False,
                times=times,
                #sampling_method="random",# for test
                sampling_method=sampling_method,
            )
    ...
    def compute_logprobs_trajectories(self, batch: Batch, backward: bool = False):
        ...
        if backward:
            ...
            policy_output_b_1 = self.backward_policy_1(states_1)
            policy_output_b_2 = self.backward_policy_2(states_2)
            logprobs_states_1 = self.env.get_logprobs_1(
                policy_output_b_1, False, actions_1, parents_1, masks_b_1
            )
            logprobs_states_2 = self.env.get_logprobs_2(
                policy_output_b_2, False, actions_2, parents_2, masks_b_2
            )
            ...
            logprobs_states_1 = self.env.get_logprobs_1(
                policy_output_f_1, True, actions_1, states_1, masks_f_1
            )
            ...
            logprobs_states_2 = self.env.get_logprobs_2(
                policy_output_f_2, True, actions_2, states_2, masks_f_2
            )
            logprobs_states = torch.add(logprobs_states_1, logprobs_states_2)
    ...
    def estimate_logprobs_data(
        ...
        while init_batch < n_states:
            ...
            while envs:
                ...
                assert all(valids)
                # Add to batch
                batch.add_to_batch(envs, actions_1, actions_2, valids, backward=True, train=True)
                # Filter out finished trajectories
                envs = [env for env in envs if not env.done]
    ...
    def train(self):
        ...
        for it in pbar:
            ...
            mean_similarity = -1
            if it % 10 == 0:
                final_smiles = batch.get_terminating_states(smiles=True)[0:n_forward]
